...
Baseline at least once, checked by whether that participant's own
per-participant CSV file for the Baseline already exists.
"""

from experiments.free_recall_baseline import (
    OUTPUT_CSV_DIR,
    OUTPUT_CSV_STEM,
    run_free_recall_baseline,
)
...
from experiments.serial_recall_letter_baseline import (
    OUTPUT_CSV_DIR as SERIAL_RECALL_OUTPUT_CSV_DIR,
    OUTPUT_CSV_STEM as SERIAL_RECALL_OUTPUT_CSV_STEM,
...
def participant_csv_path(output_csv_dir, output_csv_stem, participant_id):
    # The per-participant CSV file a condition writes to (see BaseExperiment.log_results).
    return output_csv_dir / f"{output_csv_stem}_{participant_id}.csv"


def has_completed_baseline(output_csv_dir, output_csv_stem, participant_id: int) -> bool:
    # A completed run creates this participant's file, so existence is enough.
    return participant_csv_path(output_csv_dir, output_csv_stem, participant_id).exists()
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                continue
            path = participant_csv_path(FAST_RATE_OUTPUT_CSV_DIR, FAST_RATE_OUTPUT_CSV_STEM, participant_id)
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(WM_TASK_OUTPUT_CSV_DIR, WM_TASK_OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        elif choice == "3":
            repetition_number = ask_repetition_number()
            if repetition_number is None:
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(PAUSE_CONTROL_OUTPUT_CSV_DIR, PAUSE_CONTROL_OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        else:
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(CHUNKING_OUTPUT_CSV_DIR, CHUNKING_OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        elif choice == "2":
            repetition_number = ask_repetition_number()
            if repetition_number is None:
            ...
            path = participant_csv_path(
                ARTICULATORY_SUPPRESSION_OUTPUT_CSV_DIR, ARTICULATORY_SUPPRESSION_OUTPUT_CSV_STEM, participant_id
            )
            print(f"Experiment complete.\nData saved to: {path}")
        elif choice == "3":
            repetition_number = ask_repetition_number()
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(TAPPING_CONTROL_OUTPUT_CSV_DIR, TAPPING_CONTROL_OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        else:
            print("Please enter a number from 1 to 4.")
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(OUTPUT_CSV_DIR, OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        elif choice == "2":
            if not has_completed_baseline(OUTPUT_CSV_DIR, OUTPUT_CSV_STEM, participant_id):
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"The experiment could not be completed: {error}")
                continue
            path = participant_csv_path(SERIAL_RECALL_OUTPUT_CSV_DIR, SERIAL_RECALL_OUTPUT_CSV_STEM, participant_id)
            print(f"Experiment complete.\nData saved to: {path}")
        elif choice == "2":
            if not has_completed_baseline(SERIAL_RECALL_OUTPUT_CSV_DIR, SERIAL_RECALL_OUTPUT_CSV_STEM, participant_id):
